...
/*
2． 新建 ValidCodeServlet（配置 url-pattern 为 validCode)用于生成、保存和返回图片验证码。

1) 验证码为 4 位随机数。有兴趣的同学也可将验证码设置为 4 位随机字符，字符可为大写字母、小
写字母和数字，并剔除容易混淆的大小写字母 O、小写字母 L、数字 1、数字 0 等字符。
2) 将随机生成的验证码保存至会话范围内。
3) 以图片的方式将验证码响应至登录界面中的<img>标签处。
 //String random = drawRandomNum((Graphics2D) g,"ch");//生成中文验证码图片
        ...
        //String random = drawRandomNum((Graphics2D) g, CreateCode);//根据客户端传递的CreateCode标识生成验证码图片
          //1.在内存中创建一张图片
        String CreateCode = request.getParameter("CreateCode");//接收客户端传递的CreateCode标识
        ...
        String random = drawRandomNum((Graphics2D) g, CreateCode);//根据客户端传递的CreateCode标识生成验证码图片
        //7.将随机数存在session中
        request.getSession().setAttribute("CreateCode", random);
        ...
        //画随机字符
String drawRandomNum(Graphics2D g, String... CreateCode) {
        // 设置颜色
        ...
        //CreateCode[0]==null表示没有传递参数
        if (CreateCode.length > 0 && null != CreateCode[0]) {
            if (CreateCode[0].equals("ch")) {
                // 截取汉字
                return createRandomChar(g, baseChineseChar);
            } else if (CreateCode[0].equals("nl")) {
                // 截取数字和字母的组合
                return createRandomChar(g, baseNumLetter);
            } else if (CreateCode[0].equals("n")) {
                // 截取数字
                return createRandomChar(g, baseNum);
            } else if (CreateCode[0].equals("l")) {
                // 截取字母
...
public class ValidCodeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        setBorder(g);
        drawRandomLine(g);
        String random = drawRandomNum((Graphics2D) g, CreateCode);
        request.getSession().setAttribute("CreateCode", random);
        response.setContentType("image/jpeg");
        response.setDateHeader("expries", -1);
        response.setHeader("Cache-Control", "no-cache");
